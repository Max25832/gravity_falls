...
This repository provides an R-based framework for unsupervised anomaly detection in satellite image time series, implementing the method presented in our paper “Early Detection of Forest Disturbances in Homogeneous Stands”.

## Structure 
This Approach applies a LSTM based autoencoder for unsupervised anomaly detection in homogeneous forests. The Model is trained on Sentinel-2 L2A time series of healthy Norway Spruce stands.

The workflow is structured as followed:

- run the scripts: ```R/libraries.R``` and ```R/utilities.R``` 
- run the script: ```R/data_preparation.R``` for creating ready to use time series from GEE data
- run the script: ```R/main.R``` for model initialization and training 
- run the script: ```R/model_inference.R``` for model inference on test data 
- run the script: ```R/PD_score.R``` for Applying the Pre-Defoliation-Score that we established in our Paper  

## Citation 
paper at: https://ieeexplore.ieee.org/document/11141690.
